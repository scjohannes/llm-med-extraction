...
Imaging modality: How many PET-CT scans?
...
Gender: What was the gender distribution?

```{r n-sex}

```

Age: What was the age distribution?

```{r n-age}

```

Diagnosis: What are the ten most common diagnoses?
...
Metastasis: Of how many extracted metastasis-location, did patients have a metastasis?
...
Response to treatment: How many patients had response, SD, progression?
...
Tumor-free status: How many patients had tumor-free status yes, no or na?
...
Generally: Median report length with IQR:
...
Confusion Matrix for Progression vs. Other

```{r confusion-matrix-prog-human}
#| echo: false
...
Confusion Matrix for Metastasis Yes/No.

```{r confusion-meta-human}
#| echo: false
...
```{r model-predictions-comparisons-diagnosis-tumorfree-llm}
#| echo: false
#| output: false
...
```{r model-predictions-comparisons-overall-acc-llm}
#| echo: false
#| output: false
...
gt::gtsave(
  pred_tbl_2,
  filename = "Accuracy_table_secondary.docx",
  path = here::here("output", "oncology", "suppfigures")
)
```
...
#| fig-width: 8
#| fig-height: 6
#| fig-cap: "Confusion matrices per model (grp) task: metastasis"
#| echo: false
...
p_cm <- ggplot(cm_tidy, aes(x = .pred, y = .truth, fill = pct)) +
  ...
  coord_equal() +
  labs(
    title = "Confusion matrices per model (grp) - Metastasis",
    x = "Predicted", y = "Truth"
  ) +
...
#| fig-width: 8
#| fig-height: 6
#| fig-cap: "Confusion matrices per model (grp), task: progression vs. other"
#| echo: false
...
p_cm_prog <- ggplot(cm_prog, aes(x = Extracted, y = Truth, fill = pct)) +
  geom_tile(color = "grey90") +
  geom_text(aes(label = sprintf("%d\n(%.1f%%)", n, 100 * pct)), lineheight = 0.9) +
  scale_fill_gradient(low = "#f0f4ff", high = "#2b59c3", labels = percent, name = "Percent") +
  facet_wrap(~ grp) +
  coord_equal() +
  labs(
    title = "Confusion matrices per model (grp) — Response to Treatment",
...
## Export and explore mistakes

### gpt:oss mistakes in analysis_sample(n = 300) 

```{r mistakes-gptoss}
#| echo: false
#| output: false

gpt_oss_mistakes <- analysis_sample |>
...
What mistakes did gptoss make in response to treatment task?
...
What combinations of errors did gptoss make in response to treatment task?
...
error_combos
```
### All LLM mistakes in prompt-optimization set
