...
format: 
  typst:
    ...
    include-in-header:
      text: |
        // --- Global Body Text Settings ---
        // Times New Roman, 10pt, Single Spaced (leading roughly 0.65em)
        #set text(font: "Times New Roman", size: 10pt)
        #set par(leading: 0.65em, justify: true) 

        // --- Table & Figure Settings ---
        // Tables: Arial, 9pt
        #show table: set text(font: "Arial", size: 9pt)
        
        // Captions: Arial, 8pt, Single Spaced
        #show figure.caption: set text(font: "Arial", size: 8pt)
...
#| tbl-cap: "Accuracy with 95% confidence intervals for metastasis status and reponse to treatment, by human extractors and LLMs, along with comparisons to human performance with 90% confidence intervals."
#| echo: false

pred_data <- avg_predictions(
  m_primary_outcome, by = c("grp", "task"), type = "response", vcov = ~ ier_bk
) |>
  as_tibble() |>
  select(
    task, 
...
comp_data <- avg_comparisons(
  m_primary_outcome, variable = "grp", by = "task", type = "response",
...
tab_data |>
  ...
  # --- Formatting: Accuracy (Percent) ---
  fmt_percent(
    columns = c(acc_est, acc_low, acc_high),
    decimals = 1 # Matches your %.0f preference (change to 1 if needed)
  ) |>
  cols_merge(
    columns = c(acc_est, acc_low, acc_high),
    pattern = "{1} ({2}–{3})" # En-dash separator
  ) |>
  
  # --- Formatting: Difference (Percentage Points) ---
  fmt_number(
    columns = diff_est,
    decimals = 1,
    scale_by = 100,
  ...
  # --- Cleanup: Handling the "Human" Reference Row ---
  # The join created NAs for comparison cols. We replace those with empty strings.
  sub_missing(
  ...
  cols_label(
    grp = "",
    acc_est = "Accuracy (95% CI)",
    diff_est = "Difference (pp) (90% CI)",
    p_val = "P-value NI"
  ) |>
...
pred_tbl_2 <- tab_data |>
  ...
  cols_label(
    grp = "",
    acc_est = "Accuracy (95% CI)",
    diff_est = "Difference (pp) (90% CI)", # Updated label to reflect mixed CIs
    p_val = "P-value NI"
  ) |>
  tab_options(data_row.padding = px(4))
