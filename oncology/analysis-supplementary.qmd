...
rtt_boot_summary_2 <- boot_results_rtt |>
  as_tibble() |>
  # Group by pet_ct as well now
...
# Determine order of 'value' based on Human performance on CT scans
human_order_rtt <- rtt_boot_summary_2 |>
  filter(grp == "human", pet_ct == "CT") |> 
  arrange(point.estimate) |>
  pull(value) |>
  as.character()

all_values_rtt <- rtt_boot_summary_2 |> distinct(value) |> pull(value) |> as.character()
...
```{r}
#| echo: false
#| tbl-cap: "Accuracy (%) with 95% bootstrap confidence intervals (1000 repetitions) for Response to Treatment classification by human extractors and LLMs. PET‑Scan partial and complete responses were combined into a single “Response” category."
#| tbl-colwidths: [20, 20, 20, 20, 20]

tbl_data_rtt <- rtt_boot_summary |>
  mutate(
    grp = factor(grp, levels = ord),
    value = factor(value, levels = c(human_order_rtt, setdiff(all_values_rtt, human_order_rtt)))
  ) |>
  arrange(value, grp) |>
  select(value, grp, point.estimate, conf.low, conf.high)

# Build gt table similar to other eTables
tbl_rtt <- tbl_data_rtt |>
  gt(groupname_col = "value") |>
  fmt_percent(
    columns = c(point.estimate, conf.low, conf.high),
    decimals = 1
  ) |>
  cols_merge(
    columns = c(point.estimate, conf.low, conf.high),
  ...
  cols_label(
    grp = "",
    point.estimate = md("Accuracy <br> % (95% CI)")
  ) |>
  tab_options(data_row.padding = px(4))
...
These are prompts used in the classiciations pipelines. The prompts are better understood in the conext of the code. We encourage the reader to examine the code together with the prompts [here](https://github.com/scjohannes/llm-med-extraction).

### Metastasis Classification {#sec-metastasis}
...
### Response to Treatment Classification {#sec-response}
