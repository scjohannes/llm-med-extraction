...
if(redownload){
  ...
  data <- readRDS("../data/oncology/redcap_export_complete.rds")
}
```
...
long_data <- data |>
  ...
  select(-c(oncology_radiology_extraction_complete, justification, model, prompt, sample_complete)) |> 
  ungroup()
...
if(any(is.na(analysis_data$value))) {
...
#| echo: false
#| output: false
all_ids <- na.omit(unique(analysis_data$extractor_id))
md <- c("DH", "AIM", "BT", "AMS")
...
```{r n-ier_bk}
#| echo: false
n_distinct(analysis_data$ier_bk)
```
...
model_acc_diag <- glm(
    correct ~ value,
    data = analysis_data |> filter(task == "Diagnosis") |> mutate(value = fct_lump_min(value, min = 10)),
    family = binomial(link = "logit")
  )
```
...
model_rsp_trt <- glm(
    correct ~ value,
    data = analysis_data |> filter(task == "Response to Treatment", pet_ct == FALSE),
    family = binomial(link = "logit")
  )
...
model_rsp_trt_pet <- glm(
    correct ~ value,
    data = analysis_data |> filter(task == "Response to Treatment", pet_ct == T),
    family = binomial(link = "logit")
  )
...
CI go above 1, which shouldn't happen. Maybe because of delta method used by `marginaleffects`?
Stable Disease seems to have been impossible to extract correctly for PET-CT scans (?)
```{r}
#| echo: false
#| warning: false
#| fig-width: 8
...
```{r}
#| echo: false
progression_datat <- analysis_data |> 
  filter(task == "Response to Treatment") |> 
  mutate(
...
model_progr_ppv <- glm(
    truth ~ 1,
    data = analysis_data |> filter(task == "Response to Treatment", value == "Progression") |> 
...
model_meta_sens_spec <- glm(
    ...
    data = analysis_data |> filter(str_detect(item, "metastasis")) |> mutate(truth = if_else(truth == "Yes", "Sensitivity", "Specificity")),
    family = binomial(link = "logit")
  )
...
model_ppv_npv_simple <- glm(
    truth ~ value,
    data = analysis_data |> filter(task == "Metastasis") |> mutate(truth = if_else(truth == "Yes", 1, 0)),
    family = binomial(link = "logit")
  )
...
model_ppv_simple <- glm(
    truth ~ 1,
    data = analysis_data |> filter(str_detect(item, "metastasis"), value == "Yes") |> mutate(truth = if_else(truth == "Yes", 1, 0)),
    family = binomial(link = "logit")
  )

model_npv_simple <- glm(
    truth ~ 1,
    data = analysis_data |> filter(str_detect(item, "metastasis"), value == "No") |> mutate(truth = if_else(truth == "Yes", 1, 0)),
    family = binomial(link = "logit")
  )
