...
ggplot(ap, aes(x = xlab, y = estimate, color = grp)) +  
  geom_point(position = pd, size = 2) +
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high),
                position = pd, width = 0.10) +
  scale_x_discrete(limits = "overall") +
  scale_y_continuous(
  limits = c(0.75, 1),
  breaks = seq(0.75, 1, by = 0.05)) +
  scale_color_manual(values = okabe_ito_named, breaks = ord_legend, drop = FALSE) +
  labs(x = "", y = "Probability of Correct Response", color = "") +
  theme_medical
```
