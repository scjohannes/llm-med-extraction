...
dfrtt <- avg_comparisons(m_acc, newdata = optim_sample |> filter(task == "Response to Treatment"), variable = "grp", by = "task", type = "response", vcov = ~ier_bk, equivalence = c(-0.05, Inf), conf_level = 0.9) |> 
  data.frame()
...
### model, predictions and comparisons: task diagnosis, tumorfree
...
### model, predictions and comparisons: overall accuracy
```{r}
m_acc3 <- glm(
    correct ~ grp * task,
    data = optim_sample, 
...
Version 3. bootstrap using marginaleffects
```{r}
set.seed(42)
sensspec_prog_boot <- avg_predictions(
  model_prog_sens_spec,
  by   = c("truth","grp"),
  type = "response"
) |>
...
# 3) Tabelle
gt(sensspec_prog_boot, groupname_col = "truth") |>
  cols_label(
    truth    = "Metric",
    grp      = "Group",
    Estimate = "Estimate (95% CI)"
