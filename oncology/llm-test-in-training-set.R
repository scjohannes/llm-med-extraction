...
for (i in 1:length(models)) {
  ...
  if (extract_reponse) {
    ...
    if (all(llm_response_trt_formatted$redcap_event_name == event_i) == TRUE) {
  ...
  if (extract_metastasis) {
    ...
    # Check that we only upload to desired event

    for (j in 1:length(list_to_import)) {
      if (all(list_to_import[[j]]$redcap_event_name == event_i) == TRUE) {
        REDCapR::redcap_write(
          list_to_import[[j]],
          overwrite_with_blanks = FALSE,
          redcap_uri = Sys.getenv("redcap_url_fxdb"),
