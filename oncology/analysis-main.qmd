...
```{r}
ira <- data |> 
  filter(redcap_event_name == "Extraction 1" | redcap_event_name == "Extraction 2") |> 
  pivot_wider(
    id_cols = c(ier_bk, task, item),
    names_from = redcap_event_name,
    values_from = value
  ) |> 
  mutate(
    agree = if_else(`Extraction 1` == `Extraction 2`, 1, 0)
  )
...
ira_summary <- ira |>
  ...
  summarise(
    ...
    kappa     = irr::kappa2(cbind(`Extraction 1`, `Extraction 2`))$value
  )
...
diag_counts_recoded <- data |>
  filter(!str_detect(redcap_event_name, "LLM")) |> 
  filter(task == "Diagnosis") |>
  distinct(ier_bk, truth) |>
  mutate(
    truth_recoded = if_else(
      truth %in% gi_dx,
      "Gastrointestinal tumors",
      truth
    )
  ) |>
  count(truth_recoded, sort = TRUE, name = "n")

diag_top4_other2 <- diag_counts_recoded |>
  (\(df) {
    top4   <- slice_head(df, n = 4)
    other_n <- sum(df$n) - sum(top4$n)
    ...
    out <- if (other_n > 0) {
      bind_rows(
        top4,
        tibble(truth_recoded = "Other*", n = other_n)
      )
    } else {
      top4
    }
    ...
    out |>
      mutate(
        pct   = round(n / sum(df$n), digits = 2),
        Value = glue::glue("{n} ({percent(pct)})")
      )
  })() |>
  mutate(
    Variable = "Primary Diagnosis",
    Category = truth_recoded,
    Value,
    .keep = "none"
  )
...
# diag_all: overview of all primary diagnoses with counts and percentage of patients.
diag_all <- data |>
  filter(!str_detect(redcap_event_name, "LLM")) |> 
  filter(task == "Diagnosis") |>
  distinct(ier_bk, truth) |>
...
# mets_all: overview of all metastasis locations with counts and percentage of patients.
mets_all <- data |>
  filter(!str_detect(redcap_event_name, "LLM")) |> 
  filter(task == "Metastasis", truth == "Yes") |>
  mutate(
    Category = recode(
      item,
      !!!mets_labels,
...
gtsave(
  table1,
  filename = here::here("output", "oncology", "tables", "table1.docx")
)
```
...
ggsave(
  filename = here::here("output", "oncology", "figures", "figure_1.png"),
  plot = fig1, width = 8, height = 8, dpi = 300
)

ggsave(
  filename = here::here("output", "oncology", "figures", "figure_1.svg"),
  plot = fig1, width = 8, height = 8, dpi = 300
)
```
...
comp_data <- avg_comparisons(
  ...
  select(
    ...
    diff_low = conf.low, 
    diff_high = conf.high, 
    p_val = p.value.noninf
  )

# 3. Join and Order
tab_data <- pred_data |>
  left_join(comp_data, by = c("task", "grp")) |>
  mutate(grp = factor(grp, levels = ord)) |>
  arrange(task, grp)

# 4. Build Table
final_table <- tab_data |>
  gt(groupname_col = "task") |>
  
  # 1. FORMATTING DATA ==============================================
  # Accuracy: Percent with En-dash for CI
  fmt_percent(
  ...
  cols_merge(
    columns = c(acc_est, acc_low, acc_high),
    pattern = "{1} ({2}–{3})" # Uses en-dash
  ) |>
  
  # Difference: Number (scaled to pp) with proper Minus Sign
  fmt_number(
  ...
  # P-values: Reverted to your original preference (standard 0.001 format)
  fmt(
    columns = p_val,
    fns = scales::label_pvalue(accuracy = 0.001) 
  ) |>

  # 2. HANDLING THE REFERENCE ROW ===================================
  # Clean up the "Human" row to say [Reference] instead of empty/NA
  sub_missing(
  ...
  text_transform(
    ...
    fn = function(x) "[Reference]"
  ) |>
  
  # 3. HEADER STRUCTURE =============================================
  # Rename columns using Markdown for line breaks and bolding
  cols_label(
    grp = "Model",
    acc_est = md("% (95% CI)"),
    diff_est = md("Percentage Points (90% CI)"),
    p_val = md("Noninferiority")
  ) |>
  
  # Add the Spanners (The top-level headers)
  tab_spanner(
    label = md("**Accuracy**"),
    columns = c(acc_est)
  ) |>
  tab_spanner(
    label = md("**Difference from Human**"),
    columns = c(diff_est)
  ) |>
    tab_spanner(
    label = md("**P Value**"),
  ...
  cols_align(
    align = "center",
    columns = c(acc_est, diff_est, p_val)
  ) |>
  cols_align(
    align = "left",
    columns = grp
  ...
  opt_table_font(
    font = list(google_font("Arimo"), "Arial", "Helvetica")
  ) |>
  tab_options(
...
gtsave(final_table, filename = here::here("output", "oncology", "tables", "table_2.docx"))
```
...
# --- PLOT 1: Diagnosis -------------------------------------------------------
p1 <- plot_predictions(m_acc2, newdata = analysis_sample |> filter(task == "Diagnosis") |> mutate(grp = factor(grp, levels = ord)), 
...
p2 <- plot_predictions(m_acc2, newdata = analysis_sample |> filter(task == "Radiologically Tumor Free") |> mutate(grp = factor(grp, levels = ord)), 
                       by = c("task", "grp"), type = "response", vcov = ~ier_bk) +
  scale_y_continuous(
    limits = c(0, 1),
    breaks = seq(0, 1, by = 0.2)) +
...
ap <- avg_predictions(
  ...
  mutate(xlab = "overall") |> mutate(grp = factor(grp, levels = ord))  
...
# --- COMBINE ----------------------------------------------------------------
combined_Acc <- p1 + p2 + p3 + plot_annotation(tag_levels = "A") + 
  plot_layout(guides = "collect") + 
  plot_annotation(theme = theme(legend.position = "bottom"))
...
ggsave(
  filename = here::here("output", "oncology", "figures", "secondary_accuracy.svg"),
  plot = combined_Acc,
  width = 8, height = 7, dpi = 300
)
```
...
Difference vs. human for secondary endpoints
...
dfd$contrast   <- factor(dfd$contrast,   levels = rev(contrast_lvls))
dfrtf$contrast <- factor(dfrtf$contrast, levels = rev(contrast_lvls))
dfover$contrast<- factor(dfover$contrast,levels = rev(contrast_lvls))
...
ggsave(
  filename = here("output", "oncology", "figures", "secondary_contrast.svg"),
  plot = combined_sec,
  width = 8, height = 10.5, dpi = 300
)
```
...
ggsave(
  filename = here::here("output", "oncology", "figures", "bar_plot_meta.svg"),
  plot = bar_meta,
  width = 8, height = 7, dpi = 300
)
```
